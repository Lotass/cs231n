...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params['W2'] = weight_scale * \
            np.random.randn(num_filters * H // 2 * W // 2, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, dprobs = softmax_loss(a3, y)
        loss += self.reg * np.square(self.params['W3']).sum() * 2
        loss += self.reg * np.square(self.params['W2']).sum() * 2
        loss += self.reg * np.square(self.params['W1']).sum() * 2

        da3, dw3, db3 = affine_backward(dprobs, cache3)
        da2, dw2, db2 = affine_relu_backward(da3, cache2)
        da1, dw1, db1 = conv_relu_pool_backward(da2, cache1)
